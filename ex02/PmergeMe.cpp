...
static std::vector<int> validateInput(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    for (std::string inputBuffer; std::getline(ss, inputBuffer, ' ');) {
      ...
      if (std::find(inputData.begin(), inputData.end(), number) !=
          inputData.end())
...
void PmergeMe::mergeVector(std::vector<std::pair<int, int>> &pairVector,
...
void PmergeMe::mergePairVector(std::vector<std::pair<int, int>> &pairVector,
                               int left, int right) {
  if (left < right) {
...
PmergeMe::binarySearchVector(std::vector<int> sorted, int left, int right,
                             int key) {
  if (right <= left) {
    if (key > sorted[left])
      return left + 1;
...
void PmergeMe::mergeInsertionSortVector() {
  ...
  if (_vectorSequence.size() > 1) {
    ...
    size_t jacobIndex = 2;
    for (std::vector<std::pair<int, int>>::size_type prevPairIndex = 0;
         prevPairIndex < pairSequence.size(); jacobIndex++) {
      ...
      for (; curPairIndex > prevPairIndex; curPairIndex--) {
        std::vector<int>::size_type insertIndex =
            binarySearchVector(sortedSequence, 0, sortedSequence.size() - 1,
                               pairSequence[curPairIndex].second);
        sortedSequence.insert(sortedSequence.begin() + insertIndex,
                              pairSequence[curPairIndex].second);
      }
      prevPairIndex = getJacobsthalNumber(jacobIndex) - 1;
    }
    if (remainder != 0) {
      std::vector<int>::size_type insertIndex = binarySearchVector(
          sortedSequence, 0, sortedSequence.size() - 1, remainder);
...
void PmergeMe::mergeDeque(std::deque<std::pair<int, int>> &pairDeque, int left,
                          int mid, int right) {
...
void PmergeMe::mergePairDeque(std::deque<std::pair<int, int>> &pairDeque,
                              int left, int right) {
  if (left < right) {
    int middle = left + (right - left) / 2;
...
std::deque<int>::size_type PmergeMe::binarySearchDeque(std::deque<int> sorted,
                                                       int left, int right,
                                                       int key) {
  if (right <= left) {
    if (key > sorted[left])
...
void PmergeMe::mergeInsertionSortDeque() {
  ...
  if (_vectorSequence.size() > 1) {
    ...
    sortedSequence.insert(sortedSequence.begin(), pairSequence[0].second);
    size_t jacobIndex = 2;
    for (std::deque<std::pair<int, int>>::size_type prevPairIndex = 0;
         prevPairIndex < pairSequence.size(); jacobIndex++) {
      size_t curPairIndex = getJacobsthalNumber(jacobIndex) - 1;
      if (curPairIndex >= pairSequence.size())
        curPairIndex = pairSequence.size() - 1;
      for (; curPairIndex > prevPairIndex; curPairIndex--) {
        std::deque<int>::size_type insertIndex =
            binarySearchDeque(sortedSequence, 0, sortedSequence.size() - 1,
                              pairSequence[curPairIndex].second);
        sortedSequence.insert(sortedSequence.begin() + insertIndex,
                              pairSequence[curPairIndex].second);
      }
    ...
    if (remainder != 0) {
      std::deque<int>::size_type insertIndex = binarySearchDeque(
          sortedSequence, 0, sortedSequence.size() - 1, remainder);
      sortedSequence.insert(sortedSequence.begin() + insertIndex, remainder);
    }
...
void PmergeMe::printVectorTime() {
  std::cout << "Time to process a range of " << _vectorSequence.size()
            << " elements with std::vector : " << _vectorTime / CLOCKS_PER_SEC
            << " ms" << std::endl;
}

void PmergeMe::printDequeTime() {
  std::cout << "Time to process a range of " << _dequeSequence.size()
            << " elements with std::deque : " << _dequeTime / CLOCKS_PER_SEC
            << " ms" << std::endl;
}
...
void PmergeMe::printIsAscendingVector() {
  std::cout << "Vector sorting: " << std::boolalpha
            << is_sorted(_vectorSequence.begin(), _vectorSequence.end())
            << std::endl;
}

void PmergeMe::printIsAscendingDeque() {
  cout << "Deque sorting: " << std::boolalpha
       << std::is_sorted(_dequeSequence.begin(), _dequeSequence.end())
       << std::endl;
}
