...
const std::map<std::string, float> &BitcoinExchange::getBtcDB() {
  return BitcoinExchange::_bitcoinDB;
}
...
void BitcoinExchange::checkCsvFile(const std::string &buffer,
                                   const std::string &delim,
                                   std::pair<std::string, float> *dataPair) {
  int year;
  int month;
  int day;
  if (std::sscanf(buffer.c_str(), "%d-%d-%d,%f", &year, &month, &day,
                  &dataPair->second) != 4)
...
void BitcoinExchange::checkInputFile(const std::string &buffer,
                                     const std::string &delim,
...
void BitcoinExchange::printBitcoin(std::pair<std::string, float> inputPair) {
  std::map<std::string, float>::iterator iter =
      _bitcoinDB.lower_bound(inputPair.first);
  ...
  std::cout << inputPair.first << " => " << inputPair.second << " = "
            << inputPair.second * iter->second << std::endl;
}
...
const char *BitcoinExchange::UnsupportedDataException::what() const throw() {
  return "Unsupported Date.";
}

BitcoinExchange::BadFormatException::BadFormatException(
    const std::string &inputStr) {
  _message = "Error: Bad format data => " + inputStr;
}
...
BitcoinExchange::BadInputException::BadInputException(
    const std::string &inputStr) {
  _message = "Error: Bad input => " + inputStr;
}
